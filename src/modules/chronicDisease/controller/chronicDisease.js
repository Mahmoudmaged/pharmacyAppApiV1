...
export const getChronicDiseaseList = asyncHandler(async (req, res, next) => {
  ...
  const chronicDisease = await ChronicDiseaseModel.find({
    isDeleted: req.query.freeze == "true" ? true : false,
  }).populate([
    {
      path: "createdBy",
    },
...
export const getChronicDiseaseById = asyncHandler(async (req, res, next) => {
  const lang = req.headers.lang || "EN";
  const chronicDisease = await ChronicDiseaseModel.findById(
    req.params.chronicDiseaseId
  ).populate([
    {
      path: "createdBy",
    },
  ...
  return chronicDisease
    ? res
        .status(200)
        .json({ message: lang == "EN" ? "Done" : "تم", chronicDisease })
    : next(
        new Error(
          lang == "EN"
            ? "In-valid chronicDisease ID"
            : " لم يتم العثور علي  نتيجه ",
...
export const createChronicDisease = asyncHandler(async (req, res, next) => {
  ...
  if (
    await ChronicDiseaseModel.findOne({
      $or: [{ "disease.EN": disease.EN }, { "disease.AR": disease.AR }],
    })
  ) {
    return next(
      new Error(
        lang == "EN"
          ...
          : "عفوا يوجد بالفعل مرض مسجل بهذا الاسم",
...
export const updateChronicDisease = asyncHandler(async (req, res, next) => {
  ...
  if (!chronicDisease) {
    return next(
      new Error(
        lang == "EN" ? "In-valid chronicDisease ID" : "لم يتم العثور    ",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
  }
  if (req.body.disease) {
    if (req.body?.disease?.AR) {
      if (chronicDisease.disease.AR == req.body.disease.AR.toLowerCase()) {
        return next(
          new Error(
            lang == "EN"
              ? `Sorry cannot update chronicDisease with the old AR disease:${req.body.disease.AR}`
              : ` ${req.body.disease.AR}لا يمكن تحديث  بنفس الاسم العربي`,
            { cause: { code: 409, customCode: 1011 } }
          )
        );
      }
    ...
    if (req.body?.disease?.EN) {
      if (chronicDisease.disease.EN == req.body.disease.EN.toLowerCase()) {
        return next(
          new Error(
            lang == "EN"
              ? `Sorry  cannot update chronicDisease with the old EN disease:${req.body.disease.EN}`
              : ` ${req.body.disease.EN}لا يمكن تحديث  بنفس الاسم الانجليزي`,
            { cause: { code: 409, customCode: 1011 } }
          )
        );
      }
    ...
    if (
      ...
      return next(
        new Error(
          lang == "EN"
            ? `Duplicate chronicDisease disease`
            : "عفوا يوجد بالفعل علامه تجاريه مسجله بهذا الاسم",
          { cause: { code: 409, customCode: 1011 } }
        )
      );
    }
    ...
    chronicDisease.disease = {
      AR: req.body.disease?.AR
        ? req.body.disease?.AR
        : chronicDisease.disease.AR,
      EN: req.body.disease?.EN
        ? req.body.disease?.EN
        : chronicDisease.disease.EN,
    };
  }
...
export const deleteChronicDisease = asyncHandler(async (req, res, next) => {
  const lang = req.headers.lang || "EN";
  const chronicDisease = await ChronicDiseaseModel.findByIdAndUpdate(
    req.params.chronicDiseaseId,
  ...
  if (!chronicDisease) {
    return next(
      new Error(
        lang == "EN"
          ? "In-valid chronicDisease ID"
          : "لم يتم العثور علي  العلامه التجاريه",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
  }
