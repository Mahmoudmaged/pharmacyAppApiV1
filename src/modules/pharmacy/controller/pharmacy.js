...
export const registerPharmacy = asyncHandler(async (req, res, next) => {
  ...
  const { _id } = await pharmacyModel.create({
    name,
    email,
    phone: [{ code: phone.split(" ")[0], number: phone.split(" ")[1], mainNumber: true }],
...
export const registerBranchToPharmacy = asyncHandler(async (req, res, next) => {
  ...
  if (!headquarter) {
    return next(new Error(lang == "EN" ? "Headquarter not exist" : "لم يتم العثور علي الفرع الرئسي", { cause: { code: 409, customCode: 1011 } }));
  }

  if (headquarter.headquarter) {
    return next(new Error(lang == "EN" ? "Sorry regural branch can act as head" : "عفوا لكن لايمكن ان يتم تسجل فرع عن طريق فرع اخر يجب يكون عن طريق الفرع الرئسي فقط", { cause: { code: 409, customCode: 1011 } }));
  }
  ...
  return res.status(201).json({ message: lang == "EN" ? "Done" : "تم التسجيل  بنجاح", branch });
});
...
export const approvePharmacy = asyncHandler(async (req, res, next) => {
  ...
  const request = await PharmacyRequestModel.findOne({ pharmacyId }).populate("pharmacyId")
  if (!request) {
    return next(new Error(lang == "EN" ? "not exist" : "عفو لم يتم العثور علي هذا الطلب", { cause: { code: 404, customCode: 1004 } }))
  }
  if (request.recheck) {
    // replace files  & approve

    //check temp and  save as new
    let license = request.pharmacyId.tempLicense ? request.pharmacyId.tempLicense : null;
    let taxCard = request.pharmacyId.tempTaxCard ? request.pharmacyId.tempTaxCard : null;
    let commercialRegister = request.pharmacyId.tempCommercialRegister ? request.pharmacyId.tempCommercialRegister : null;

    const updatedPharmacy = await pharmacyModel.findOneAndUpdate({ pharmacyId }, {
      license: license ? license : request.pharmacyId.license,
      taxCard: taxCard ? taxCard : request.pharmacyId.taxCard,
      commercialRegister: commercialRegister ? commercialRegister : request.pharmacyId.commercialRegister,
    ...
    if (!updatedPharmacy) {
      return next(new Error(lang == "EN" ? "Fail to update please try agin later" : "عفو برجاء  المحاوله في وقت لاحق", { cause: { code: 400, customCode: 1015 } }))
    }


    //Remove old files
    if (license) {
      if (fs.existsSync(path.join(__dirname, `./../../../${updatedPharmacy.license}`))) {
        await unlink(path.join(__dirname, `./../../../${updatedPharmacy.license}`));
      }
    }

    if (commercialRegister) {
      if (fs.existsSync(path.join(__dirname, `./../../../${updatedPharmacy.commercialRegister}`))) {
        await unlink(path.join(__dirname, `./../../../${updatedPharmacy.commercialRegister}`));
      }
    }

    if (taxCard) {
      if (fs.existsSync(path.join(__dirname, `./../../../${updatedPharmacy.taxCard}`))) {
        await unlink(path.join(__dirname, `./../../../${updatedPharmacy.taxCard}`));
      }
    }
    // request approve
...
export const uploadPharmacyFiles = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(new Error(lang == "EN" ? "Not authorized manger" : "عفو لا تمتلك الصلاحيه", { cause: { code: 403, customCode: 1003 } }))
  }


  if (pharmacy.approved) {
    //save in temp path
    pharmacy.tempLicense = req.files?.license ? req.files.license[0].dest : null;
    pharmacy.tempCommercialRegister = req.files?.commercialRegister ? req.files.commercialRegister[0].dest : null;
    pharmacy.tempTaxCard = req.files?.taxCard ? req.files.taxCard[0].dest : null;
    pharmacy.updatedBy = req.user._id;
    await pharmacy.save()
    //pin request to review 
    await PharmacyRequestModel.findOneAndUpdate({ pharmacyId }, { updatedBy: req.user._id, recheck: true })
    return res.json({ message: lang == "EN" ? "Done , Pharmacy still working with the old certificates until system admin approve the new one" : "تم تسجيل بنجاح سيتم استمرار العمل بالوثائق القديمه الي حين موافقه الادمن علي الوثائق الجديده" })
  } else {
    //not approved yet
    // save in main path
    if (!req.files.license || !req.files.commercialRegister || !req.files.taxCard) {
      return next(new Error(lang == "EN" ? "Please upload all pharmacy certificates" : "عفوا برجاء رفع جميع الملفات ", { cause: { code: 400, customCode: 1014 } }))
    }
    ...
    if (pharmacy.license) {
      if (fs.existsSync(path.join(__dirname, `./../../../${pharmacy.license}`))) {
        await unlink(path.join(__dirname, `./../../../${pharmacy.license}`));
      }
    }

    if (pharmacy.commercialRegister) {
      if (fs.existsSync(path.join(__dirname, `./../../../${pharmacy.commercialRegister}`))) {
        await unlink(path.join(__dirname, `./../../../${pharmacy.commercialRegister}`));
      }
    }

    if (pharmacy.taxCard) {
      if (fs.existsSync(path.join(__dirname, `./../../../${pharmacy.taxCard}`))) {
    ...
    if (! await PharmacyRequestModel.findOneAndUpdate({ pharmacyId }, { updatedBy: req.user._id })) {
      ...
      return res.json({ message: lang == "EN" ? "Done , and wait for admin approval" : "تم و في انتظار موافقه الادمن" })
    }
  }
})
//pharmacy image  //Done
...
export const hireEmployee = asyncHandler(async (req, res, next) => {
  ...
  if (employee.role.title != "user") {
    return next(new Error(lang == "EN" ? `Sorry cannot add user  except users of type user ` : "عفوا ولكن لا يمكن ادراج موظف يحمل صلاحيات اخري غير مستخدم عادي", { cause: { code: 401, customCode: 1001 } }));
  }
  ...
  await pharmacyModel.findOneAndUpdate({ _id: pharmacyId }, {
...
export const pharmacyEmployeeLogin = asyncHandler(async (req, res, next) => {
  ...
  //check email exist
  const pharmacy = await pharmacyModel.findOne({ email: pharmacyEmail }).populate([
    {
      path: 'employee'
    }
  ]);
  if (!pharmacy) {
    return next(new Error(lang == "EN" ? "pharmacy  not exist" : "عفوا لم يتم العثور علي هذا الحساب برجاء التاكد من البيانات", { cause: { code: 404, customCode: 1004 } }));
  }
  ...
  if (!pharmacy.employee?.length) {
    return next(new Error(lang == "EN" ? "Not a register employee" : "عفوا لم يتم العثور علي هذا الحساب برجاء التاكد من البيانات", { cause: { code: 404, customCode: 1004 } }));
  }
  ...
  const access_token = generateToken({
    payload: { id: employee._id, role: employee.role, fullName: employee.fullName },
    expiresIn: 86400, // 1d 60*60*24
  });
...
export const requestNewPharmacyConfirmEmail = asyncHandler(async (req, res, next) => {
  const lang = req.headers.lang || "EN";
...
export const requestNewAccessTokenToPharmacy = asyncHandler(async (req, res, next) => {
  const lang = req.headers.lang || "EN";
