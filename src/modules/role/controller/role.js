...
export const createRole = asyncHandler(async (req, res, next) => {
  ...
  if (await roleModel.findOne({ title })) {
    return next(
      new Error(
        lang == "EN"
          ? `Duplicate Role  name ${title}`
          : "يوجد بالفعل صلاحيه بنفس الاسم",
...
export const addRoleItem = asyncHandler(async (req, res, next) => {
  ...
  if (!privilege) {
    return next(
      new Error(lang == "EN" ? `not exist` : "لا توجد صلاحيه بهذا اللاسم", {
...
export const deleteRoleItem = asyncHandler(async (req, res, next) => {
  ...
  if (!Role) {
    return next(
      new Error(
        lang == "EN"
          ? `In-valid  Role set item or  Role set not exist`
          : "خطاء لاتوجد الصلاحيه المطلويه",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
  }
...
export const updateRole = asyncHandler(async (req, res, next) => {
  ...
  if (title.toLowerCase() == role.title) {
    return next(
      new Error(
        lang == "EN"
          ? `Cannot update role set with the same old name: ${title}`
          : "لا يمكن التحديث باستخدم نفس الاسم",
        { cause: { code: 409, customCode: 1011 } }
      )
    );
  }
  ...
  if (await roleModel.findOne({ title })) {
    return next(
      new Error(
        lang == "EN"
          ? `Duplicate role set name: ${title}`
          : "توجد بالفعل صلاحيه تحمل نفس الاسم",
        { cause: { code: 409, customCode: 1011 } }
      )
    );
  }
  ...
  return res
    .status(200)
    .json({ message: lang == "EN" ? "Done" : "تم", updatedRole });
});
