...
export const createMedicine = asyncHandler(async (req, res, next) => {
    ...
    return next(
      new Error(
        lang == "EN"
          ...
          : "عفوا يوجد بالفعل منتج يحمل نفس الاسم",
  ...
  if (!(await Category.findById(category))) {
    return next(
      new Error(
        lang == "EN"
          ? `In-valid category `
          : "عفوا لم يتم العثور علي هذه الفئه",
  ...
  return res
    .status(201)
    .json({ message: lang == "EN" ? "Done" : "تم", medicine });
});
...
export const updateMedicine = asyncHandler(async (req, res, next) => {
  ...
  if (!medicine)
    return next(
      new Error(
        lang == "EN"
          ? "Invalid medicine id!"
          : "عفوا لم يتم العثور علي هذا المنتج",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
  ...
  if (category && !(await Category.findById(category))) {
    return next(
      new Error(
        lang == "EN"
          ? `Category not found!`
          : "عفوا لم يتعم العثور علي هذه الفئه",
        { cause: { code: 404, cause: 1004 } }
      )
    );
  }
  ...
  if (brand && !(await Brand.findById(brand))) {
    return next(
      new Error(
        lang == "EN" ? `Brand not found!` : "عفوا لا توجد  علامه تجاريه",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
  }
    ...
    if (
      ...
      return next(
        new Error(
          lang == "EN"
            ? `Duplicate medicine name`
            : "يوجد بالفعل منتج يحمل نفس الاسم من قبل",
          { cause: { code: 409, customCode: 1011 } }
        )
      );
    }
  ...
  medicine.description = {
    AR: req.body.description?.AR
      ? req.body.description?.AR
      : medicine.description.AR,
    EN: req.body.description?.EN
      ? req.body.description?.EN
      : medicine.description.EN,
  };

  //mainPrice &  discountPercent & salePrice
  medicine.mainPrice = mainPrice ? mainPrice : medicine.mainPrice;
  medicine.discountPercent = discountPercent
    ? mainPrice
    : medicine.discountPercent;
  medicine.salePrice = Number.parseFloat(
    medicine.mainPrice - medicine.mainPrice * (medicine.discountPercent / 100)
  ).toFixed(2);
  ...
  // indicationsForUse
  medicine.indicationsForUse = indicationsForUse
    ? _.uniq(indicationsForUse)
    : medicine.indicationsForUse;
...
export const updateMedicineVariant = asyncHandler(async (req, res, next) => {
  ...
  if (!variant)
    return next(
      new Error(
        lang == "EN"
          ? "Invalid variant or medicine id!"
          : "عفوا لم يتم العثور علي هذا المنتج",
        { cause: { code: 404, customCode: 1004 } }
      )
    );
    ...
    if (!(await Category.findById(categoryId))) {
      return next(
        new Error(
          lang == "EN"
            ? "Category not found!"
            : "عفو لم يتم العثور علي هذه الفئه",
          { cause: { code: 404, customCode: 1004 } }
        )
      );
    }
    ...
    if (!(await Brand.findById(brandId))) {
      return next(
        new Error(
          lang == "EN"
            ? "Brand not found!"
            : "عفو لا توجد هذه العلامه التجاريه",
          { cause: { code: 404, customCode: 1004 } }
        )
      );
    }
